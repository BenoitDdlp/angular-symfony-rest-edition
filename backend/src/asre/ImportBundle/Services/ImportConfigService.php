...
use asre\ImportBundle\Exception\AsreNotImportableException;
...
class ImportConfigService
{
  ...
  public function fromShortClassName($shortClassName, $asString = false)
  {
    ...
    {
      $return = array();
      foreach (ImportHelper::$entityConfig as $shortClassName => $entityDefinition)
      {
        try
        {
          ...
          $return[] = $importFields;
        } catch (AsreNotImportableException $e)
        {
...
//        \Doctrine\Common\Util\Debug::dump($importFields);
    ...
    { //no Importer annotation found
      throw new AsreNotImportableException("Cannot import $entityClassName : No @Importer in $entityClassName.");
    }

    if (isset($importFields[0]) && strpos($importFields[0], 'importCode') === false)
    { //ensure importCode is first
      throw new AsreNotImportableException("Cannot import $entityClassName : The property 'importCode' must be the first annotated with @Importer in $entityClassName.");
    }
