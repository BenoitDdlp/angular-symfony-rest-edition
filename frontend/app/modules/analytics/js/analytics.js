...
angular.module('analyticsApp')
  ...
  .directive('trackUiClick', function ($analytics)
  {
    return function link($scope, $elem, attrs)
    {
      ...
      {
        $analytics.eventTrack('Click [path="' + getElementPath($elem) + '"]', {
          category: 'UserInteraction',
          label: attrs.trackUiClick
        });
  ...
  .directive('trackUiChange', function ($analytics)
  {
    return function link($scope, $elem, attrs)
    {
      ...
      {
        $analytics.eventTrack('Change [path="' + getElementPath($elem) + '"][value="' + $elem.val() + '"]', {
          category: 'UserInteraction',
          label: attrs.trackUiChange
        });
  ...
  .directive('trackUiHover', function ($analytics)
  {
    return function link($scope, $elem, attrs)
    {
      $elem.hover(function ()
      {
        $analytics.eventTrack('Hover [path="' + getElementPath($elem) + '"]', {
          category: 'UserInteraction',
          label: attrs.trackUiHover
        });
  ...
  .factory('AJAXInterceptor', function ($q, $analytics)
  {
    return {
      ...
      {
        $analytics.eventTrack(rejection.config.url, {
          category: 'AJAXResponseError',
          label: '[status="' + rejection.status + '"]'
        });
