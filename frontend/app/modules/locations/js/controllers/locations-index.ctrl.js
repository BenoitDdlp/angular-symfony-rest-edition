 ...
 * @type {controller}
 */
angular.module('locationsApp').controller('locationsListCtrl', ['$scope',
  '$routeParams',
  'locationsFact',
  '$modal',
  'pinesNotifications',
  'translateFilter',
  function ($scope, $routeParams, locationsFact, $modal, pinesNotifications, translateFilter)
{
    //Prepare entity list for the locations list handling
    $scope.entities = [];

    //Declare request to execute on search query
    $scope.request = locationsFact.all;
...
//            });
    ...
    $scope.deleteModal = function (index, location)
    {
        ...
        var modalInstance = $modal.open({
            templateUrl: 'modules/locations/partials/modals/locations-delete-modal.html',
